...
def estimate_sigma(irradiance: np.ndarray, w_0: float, use_gamma: bool = False, res: int = 100, plot=False):
    ...
    (alpha, beta, scale), p_cov = curve_fit(scint_func, xx, yy, p0=[1, 5, 0.7],
                                            bounds=((0, 0.5, 0), (5, 10, 1)))
    if plot:
        xx = np.linspace(1e-15, 1, 1001)
        label = 'gamma fitment' if use_gamma else 'lognormal fitment'
        plt.plot(xx, scint_func(xx, alpha, beta, scale), label=label)
...
def combined_dist(X: np.ndarray, alpha: float, beta: float, scale: float = 1):
    return [quad(lambda I: beta_func.cdf(I, alpha, beta) * probability_dist(
...
def combined_dist_gamma(X: np.ndarray, alpha: float, beta: float, scale: float = 1):
    return [quad(lambda I: beta_func.cdf(I, alpha, beta) * gamma_gamma(i, I), 0, 1)[0] / scale for i in X]
